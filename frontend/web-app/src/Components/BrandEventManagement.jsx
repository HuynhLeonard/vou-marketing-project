...
const BrandEventManagement = () => {
    ...
    const { isFetching, refetch } = useQuery("fetch-my-events", () => callApiGetMyEvents(brandId), {
        onSuccess: (data) => {
            console.log(data);
    ...
    const { isFetching: isFetchingItemsAndBrands, refetch: refetchItemsAndBrands } = useQuery(
        ...
        },
        {
            onSuccess: (data) => {
                ...
                const listAvailableBrands = dataBrandCoop.filter(
                    (brand) => brand.idUser !== brandId
                );
                    ...
                    listEvents.map((event) => {
                        ...
                        return (
                            <Card
                                ...
                                date={
                                    convertDataToOutputString(event.startDate) +
                                    " - " +
                                    convertDataToOutputString(event.endDate)
                                }
