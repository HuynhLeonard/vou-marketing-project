...
import ImageUploader from "./BrandEvent/ImageUploader";
import Tag from "./BrandEvent/Tag";
import CheckBox from "./BrandEvent/CheckBox";
...
import FormGame from "./BrandEvent/FormGame";
import { convertDataToOutput, convertInputToSave, convertDataToOutputString } from "../utils/date";
import {
    callApiGetEventDetail,
    callApiUpdateEventDetail,
    callApiUploadEventImgs,
} from "../service/event";
...
const EventDetail = () => {
    ...
    const [expiredDay, setExpiredDay] = useState(
        convertDataToOutput(dataEvent.inventoryInfo?.expiration_date)
    );
    ...
    const [gameType, setgameType] = useState(
        dataEvent.gameInfoDTO?.gameType === "shake-game" ? "Lắc xu" : "Quizz"
    );
    ...
    const [startedAt, setGameStartAt] = useState(
        convertDataToOutput(dataEvent.gameInfoDTO?.startedAt)
    );
    ...
    const { isFetching, refetch } = useQuery(
        ...
        () => callApiGetEventDetail(idEvent),
        {
            onSuccess: (data) => {
                ...
                setgameType(
                    data.metadata?.gameInfoDTO?.gameType === "shake-game" ? "Lắc xu" : "Quizz"
                );
    ...
    const toDashboard = () => {
        if (gameType === "Lắc xu") {
            navigate(`/brand/event/eventDashboard?e=${idEvent}&t=1`);
        } else {
            navigate(`/brand/event/eventDashboard?e=${idEvent}&t=2`);
        }
    };
        ...
        <div className="container w-full p-5 bg-white" data-theme="retro">
            {isLoading ? (
                ...
                <>
                    ...
                    <div className="container flex flex-col bg-base-100 shadow-md rounded-3xl py-5 px-5 my-4 gap-5 border border-gray-200">
                        <div className="flex justify-between items-center">
                            <Tag status={status} />
                            {status !== "pending" ? (
                                <div
                                    className="primary_btn_small btn btn-info brightness-125 text-gray-300"
                                    onClick={toDashboard}
                                >
                                    Xem báo cáo
                                </div>
                        ...
                        >
                            ...
                            {isEventForm ? (
                                <div>
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <div className="flex flex-col px-2 py-2 grow">
                                            ...
                                            <input
                                                disabled={status === "done" || gameType === "Quizz"}
                                                type="number"
                                                className={`${
                                                    status === "done" || gameType === "Quizz"
                                                        ? "input_text_disabled"
                                                        : "input_text"
                                                } input input-bordered input-info`}
                                                placeholder="100"
                                                ...
                                                value={numOfVouchers || ""}
                                                onChange={(e) => setNumOfVouchers(e.target.value)}
                                                required
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col px-2 py-2">
                                        <h5 className="text-base font-semibold">
                                            Hợp tác với brand khác (Nếu có)
                                        </h5>
                                        <div className="flex gap-4 mt-2">
                                            {listAvailableBrands.map((brand, index) => (
                                                <CheckBox
                                                    key={`${brand.idUser}-${listBrands.includes(
                                                        brand.idUser
                                                    )}`}
                                    ...
                                    <h2 className="text-heading3_semibold text-primary mt-8">
                                        Voucher
                                    </h2>
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <div className="flex flex-col px-2 py-2 grow">
                                            ...
                                            <input
                                                ...
                                                type="text"
                                                className={
                                                    "input_text_disabled input input-bordered input-info"
                                                }
                                                placeholder="XXXXXX"
                                                name="voucher_code"
                                                value={voucherCode || ""}
                                    ...
                                    <div className="flex gap-4">
                                            ...
                                            {status === "done" ? (
                                                ...
                                                <div
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        changeCategoryVoucher(openCategoryVoucher);
                                                    }}
                                                >
                                        ...
                                        <div className="flex flex-col px-2 py-2 grow">
                                            ...
                                            <DatePicker
                                                ...
                                                selected={expiredDay}
                                                minDate={new Date()}
                                                onChange={(date) => setExpiredDay(date)}
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col px-2 py-2 h-[200px]">
                                        ...
                                        <textarea
                                            ...
                                            className={`${
                                                status === "done"
                                                    ? "input_text_disabled h-full"
                                                    : "input_text h-full"
                                            } input input-bordered input-info`}
                                            placeholder="Mô tả ngắn gọn về cách sử dụng voucher"
                                    ...
                                    <div className="flex flex-col px-2 py-2">
                                        ...
                                        <ImageUploader
                                            image={voucherImg}
                                            setResource={setvoucherImg}
                                            isDisabled={status === "done"}
                                        />
                                    </div>

                                    <div
                                        className="primary_btn w-[200px] mt-8 btn btn-info brightness-125 text-gray-300"
                                        onClick={() => changeForm(isEventForm)}
                                    >
                                        Tiếp theo
                                    </div>
                                </div>
                                ...
                                /* Tro choi */
                                <div>
                                    <h2 className="text-heading3_semibold text-primary">
                                        Trò chơi
                                    </h2>
                                    <div className="flex gap-4">
                                        <div className="flex flex-col px-2 py-1 min-w-[424px] ">
                                            ...
                                            <input
                                                disabled
                                                type="text"
                                                className="input_text_disabled input input-bordered input-info"
                                                name="game_type"
                                                value={gameType}
                                                required
                                            />
                                    ...
                                    {/* Nội dung trò chơi */}
                                    <h2 className="text-heading3_semibold text-primary mt-8">
                                        Phần nội dung trò chơi
                                    </h2>
                                    {gameType === "Quizz" ? (
                                        <>
                                            <div className="flex flex-col px-2 py-2 max-w-[424px]">
                                                <h5 className="text-base font-semibold">
                                                    Thời gian bắt đầu chơi game
                                                </h5>
                                                <DatePicker
                                                    placeholderText="dd/mm/yyyy"
                                                    disabled
                                                    className="input_text_disabled w-full input input-bordered input-info"
                                                    dateFormat="dd/MM/yyyy h:mm aa"
                                                    locale={vi}
                                                    selected={startedAt}
                                        ...
                                        <div className="flex flex-col px-2 py-2 mb-2">
                                            <div className="flex flex-col px-2 py-2">
                                                <h5 className="text-base font-semibold">
                                                    Chọn vật phẩm cho game
                                                </h5>
                                                <span className="text-medium font-light italic tex-gray-700">
                                                    Hệ thống sẽ cho phép tạo ra các vật phẩm sau
                                                </span>
                                                <div className="flex gap-4 mt-2">
                                            ...
                                            <div className="flex flex-col px-2 py-2 ">
                                                <h5 className="text-base font-semibold ">
                                                    Số lượng xu cần đạt để đổi voucher (Nếu có chọn
                                                    item xu)
                                                </h5>
                                                <input
                                                    disabled={status === "done" || !hasItemCoin}
                                                    type="text"
                                                    className={
                                                        status === "done" || !hasItemCoin
                                                            ? `input_text_disabled max-w-[404px] input input-bordered input-info`
                                                            : `input_text max-w-[404px] input input-bordered input-info`
                                                    }
                                                    placeholder=""
                                                    name="aim_coin"
                                                    defaultValue={dataEvent.inventoryInfo?.aim_coin}
                                                    required
                                                />
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex gap-4 w-[422px]">
                                        <div
                                            className="outline_btn w-[200px] mt-8 btn btn-error text-white"
                                            onClick={() => changeForm(isEventForm)}
                                        >
                                            Trở lại
                                        </div>
                                        {status === "done" ? null : (
                                            <div
                                                className="primary_btn w-[200px] mt-8 btn btn-success text-gray-300 brightness-105"
                                                onClick={() => sendData()}
                                            >
                                                Lưu thay đổi
                                            </div>
