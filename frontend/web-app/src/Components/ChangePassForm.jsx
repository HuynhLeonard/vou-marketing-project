...
import { callApiChangePassword } from "../service/user";
...
const ChangePassForm = ({ handleClose }) => {
    ...
    const changePassMutation = useMutation((data) => callApiChangePassword(data), {
        onSuccess: (data) => {
            console.log(data);
            // noti();
            setIsError(false);
    ...
    const validate = (formProps) => {
        ...
        if (!accountRegex.test(formProps.password)) {
            setNotiMsg(
                "Mật khẩu bao gồm 8 ký tự trở lên bao gồm chữ hoa, chữ thường, số, ký hiệu (ví dụ: !@#$)"
            );
            setIsError(true);
            setShowNoti(true);
            return false;
    ...
    return (
        <div
            className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50 font-Kanit"
            data-theme="retro"
        >
            <div
                className={`${showNoti ? "" : "hidden"} absolute flex flex-row justify-center z-50`}
            >
                <Notification
            ...
            <div className="relative bg-base-100 p-5 rounded-lg shadow-lg w-full max-w-xl h-auto mx-4">
                <div className="flex mb-2">
                    <h4 className="text-heading3_semibold font-bold text-2xl text-info">
                        Thay đổi mật khẩu
                    </h4>
                    <button
                ...
                >
                    ...
                    <div
                        className="primary_btn btn btn-info mt-5 text-center"
